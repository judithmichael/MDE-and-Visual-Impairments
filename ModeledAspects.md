...
- S24 lists the relation between WCAG guidelines and model types of WebML in the text but without providing examples for concrete models. It includes a presentation model for the arrangement of elements in the UI and a personalization model for content features
of each user or user groups.
...
- S6 mentions that they model the context, users, user preferences, UI and requirements ("The system automatically model and store these requirements and annotated with metadata related to device usage and contextual recommendations into a UI adaptation ontology."), however, details are missing.
- S30 uses a user, navigation, functional, UI and representation model without providing further details about the modeling concepts. 
